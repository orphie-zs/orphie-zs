...
const GenerationContext = struct {
    ...
    fn getFieldXorConstant(_: *Self, field: descriptor.FieldDescriptorProto) ?u16 {
        const options = field.options orelse return null;
        const value = options.xor orelse return null;
        return @intCast(value);
    }

    fn getMessageCmdId(_: *Self, message: descriptor.DescriptorProto) u16 {
        const options = message.options orelse return 0;
        return options.cmd_id orelse 0;
    }
    ...
    fn generateFieldDescriptor(ctx: *Self, list: *std.ArrayList(string), fqn: FullName, file: descriptor.FileDescriptorProto, message: descriptor.DescriptorProto, field: descriptor.FieldDescriptorProto, is_union: bool) !void {
        ...
        const xor_const = ctx.getFieldXorConstant(field);
        const format = "        .{s} = fd({?d}, {?d}, {s}),\n";
        try list.append(try std.fmt.allocPrint(allocator, format, .{ name, field.number, xor_const, descStr }));
    }
    ...
    fn generateMessages(ctx: *Self, list: *std.ArrayList(string), fqn: FullName, file: descriptor.FileDescriptorProto, messages: std.ArrayList(descriptor.DescriptorProto)) !void {
        for (messages.items) |message| {
            ...
            const cmd_id = ctx.getMessageCmdId(m);

            try list.append(try std.fmt.allocPrint(allocator, "\npub const {?s} = struct {{\n    pub const CmdId = {};\n", .{ m.name.?.getSlice(), cmd_id }));
            ...
            for (m.oneof_decl.items, 0..) |oneof, i| {
                ...
                if (union_element_count > 1) {
                    const oneof_name = oneof.name.?.getSlice();
                    try list.append(try std.fmt.allocPrint(allocator, "    .{s} = fd(null, null, .{{ .OneOf = {s}_union }}),\n", .{ oneof_name, oneof_name }));
                }
            }
