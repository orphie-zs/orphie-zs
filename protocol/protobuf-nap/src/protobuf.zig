...
pub fn fd(comptime field_number: ?u32, comptime xor_constant: ?u16, comptime ftype: FieldType) FieldDescriptor {
...
fn encode_zig_zag(int: anytype, xor_constant: u16) u64 {
...
fn append_as_varint(pb: *ArrayList(u8), int: anytype, xor_constant: ?u16, comptime varint_type: VarintType) Allocator.Error!void {
    const type_of_val = @TypeOf(int);
    const val: u64 = blk: {
        switch (@typeInfo(type_of_val).int.signedness) {
            .signed => {
                switch (varint_type) {
                    .ZigZagOptimized => {
                        break :blk encode_zig_zag(int, xor_constant);
                    },
                    .Simple => {
                        var value: u64 = @bitCast(@as(i64, @intCast(int)));
                        if (xor_constant) |xor| {
                            value ^= @intCast(xor);
                        }
                        break :blk value;
...
fn append_varint(pb: *ArrayList(u8), value: anytype, xor_constant: ?u16, comptime varint_type: VarintType) Allocator.Error!void {
    switch (@typeInfo(@TypeOf(value))) {
        .@"enum" => try append_as_varint(pb, @as(i32, @intFromEnum(value)), xor_constant, varint_type),
        .bool => try append_as_varint(pb, @as(u8, if (value) 1 else 0), xor_constant, varint_type),
        .int => try append_as_varint(pb, value, xor_constant, varint_type),
...
fn append_const_bytes(pb: *ArrayList(u8), value: ManagedString) Allocator.Error!void {
    ...
    try append_as_varint(pb, slice.len, null, .Simple);
...
fn append_packed_list_of_varint(pb: *ArrayList(u8), value_list: anytype, comptime field: FieldDescriptor, comptime varint_type: VarintType) Allocator.Error!void {
    if (value_list.items.len > 0) {
        ...
        const len_index = pb.items.len;
        for (value_list.items) |item| {
            try append_varint(pb, item, null, varint_type);
        }
        const size_encoded = pb.items.len - len_index;
        try insert_raw_varint(pb, size_encoded, len_index);
    }
...
fn append_tag(pb: *ArrayList(u8), comptime field: FieldDescriptor) Allocator.Error!void {
    ...
    try append_varint(pb, tag_value, null, .Simple);
}
    ...
    switch (field.ftype) {
        .Varint => |varint_type| {
            if (!is_default_scalar_value or force_append) {
                try append_tag(pb, field);
                try append_varint(pb, value, field.xor_constant, varint_type);
            }
        },
        .FixedInt => {
            if (!is_default_scalar_value or force_append) {
...
fn decode_value(comptime decoded_type: type, comptime ftype: FieldType, comptime xor_constant: ?u16, extracted_data: Extracted, allocator: Allocator) UnionDecodingError!decoded_type {
    return switch (ftype) {
        .Varint => |varint_type| switch (extracted_data.data) {
            .RawValue => |value| {
                var varint_value = (try decode_varint_value(decoded_type, varint_type, value));
                if (xor_constant) |xor| {
                    varint_value ^= @as(decoded_type, xor);
                }
                return varint_value;
            },
    ...
    switch (field_desc.ftype) {
        .Varint, .FixedInt, .SubMessage, .String, .Bytes => {
            ...
            switch (@typeInfo(field.type)) {
                .optional => |optional| @field(result, field.name) = try decode_value(optional.child, field_desc.ftype, field_desc.xor_constant, extracted_data, allocator),
                else => @field(result, field.name) = try decode_value(field.type, field_desc.ftype, field_desc.xor_constant, extracted_data, allocator),
            }
        },
        ...
        .OneOf => |one_of| {
            ...
            inline for (@typeInfo(one_of).@"union".fields) |union_field| {
                ...
                if (is_tag_known(v, extracted_data)) {
                    ...
                    const value = try decode_value(union_field.type, v.ftype, field_desc.xor_constant, extracted_data, allocator);
...
test "get varint" {
    ...
    defer pb.deinit();
    try append_varint(&pb, @as(i32, 0x12c), null, .Simple);
    try append_varint(&pb, @as(i32, 0x0), null, .Simple);
    try append_varint(&pb, @as(i32, 0x1), null, .Simple);
    try append_varint(&pb, @as(i32, 0xA1), null, .Simple);
    try append_varint(&pb, @as(i32, 0xFF), null, .Simple);
...
test "encode and decode multiple varints" {
    ...
    for (list) |num|
        try append_varint(&pb, num, null, .Simple);
...
test "zigzag i32 - encode" {
    ...
    try append_as_varint(&pb, @as(i32, -500), null, .ZigZagOptimized);
...
test "zigzag i64 - encode" {
    ...
    try append_as_varint(&pb, @as(i64, -500), null, .ZigZagOptimized);
    try testing.expectEqualSlices(u8, input, pb.items);
}
